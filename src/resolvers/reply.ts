...
export class ReplyResolver {
  ...
  async voteReply(
    ...
    const updoot = await Replyupdoot.findOne({ where: { replyId, userId } });
    ...
    if (updoot && updoot.value == defaultValue) {
      ...
      await AppDataSource.transaction(async (tm) => {
        await tm.query(
          `    
          insert into replyupdoot ("userId", "replyId", value)
          values ($1, $2, $3);
          `,
  ...
  async replies(
    ...
    const replies = await AppDataSource.query(
      ...
      ${
        req.session.userId
          ? '(select value from replyupdoot where "userId" = $2 and "replyId" = r.id) "voteStatus"'
          : 'null as "voteStatus"'
      }
      from reply r
  ...
  async deleteReply(
    ...
    await Replyupdoot.delete({ replyId: id });
